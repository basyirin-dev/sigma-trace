...
- Create Vite + React + TypeScript project: `npm create vite@latest giha -- --template react-ts`
...
- **M2:** Install and verify Canvas2D renders a test scene
...
- Create `opencode.json` at project root with:
  - `name: "giha"`, team metadata, UNESCO hackathon context
...
- Create `.opencode/instructions.md` with:
  - Project: GIHA, two-mode game for UNESCO Youth Hackathon 2026
  - Tech stack: React, Canvas2D, Zustand, Vite, TypeScript
...
- [x] `npm test` runs vitest with coverage
- [x] `npm run lint` passes with zero errors
- [x] GitHub Actions passes all checks on push
- [x] Netlify deploy produces live URL
- [x] Canvas2D test scene renders colored tile grid
- [x] opencode.json validates (no syntax errors)
- [x] TypeScript LSP resolves `@engine/*` path aliases
- [x] codebase-memory-mcp responds to search queries on this project
