...
function generatePostMortem(): string[] {
  ...
  if (sim.r0 > 2.0) {
    analysis.push(`R₀ reached ${sim.r0.toFixed(1)} — spread rate grew beyond intervention capacity`)
    ...
    for (const inv of antiR0Interventions) {
      ...
      if (!game.appliedInterventions.includes(inv.id)) {
        ...
        analysis.push(`Deploying "${inv.name}" (cost: $${escCost}, base $${inv.cost}) would have reduced R₀ by ${Math.abs(inv.effect.r0Delta).toFixed(1)} for ${inv.effect.durationTicks} ticks`)
      }
    }
  }

  if (sim.sigma < 30) {
    analysis.push(`σ-coherence critically low (${sim.sigma.toFixed(0)}) — population trust collapsed`)
    const proSigmaInterventions = INTERVENTIONS.filter((i) => i.effect.sigmaDelta > 0)
    for (const inv of proSigmaInterventions) {
      const useCount = game.interventionUseCounts[inv.id] ?? 0
      if (!game.appliedInterventions.includes(inv.id)) {
        const escCost = getEscalatedCost(inv.cost, useCount)
        analysis.push(`"${inv.name}" (cost: $${escCost}, base $${inv.cost}) would have boosted σ by +${inv.effect.sigmaDelta} per tick for ${inv.effect.durationTicks} ticks`)
      }
    }
  }

  if (sim.sigma < 20) {
    analysis.push(`σ-coherence entered the trap zone (below 20). Recovery requires sustained σ-boosting interventions deployed at least 15 ticks before the trap threshold.`)
  }
  ...
  if (completedCaseIds.length === 0 || completedCaseIds.every((id) => id.startsWith('__'))) {
    analysis.push('No cases were solved — disinformation spread through the city unchecked. Each solved case provides critical intelligence that strengthens the city\u2019s defenses.')
  } else {
  ...
  if (analysis.length === 0) {
    analysis.push('The collapse was sudden — no single intervention could have prevented this outcome')
  }
  ...
  return (
    <div className={styles.screen} data-testid="gameover-screen">
      ...
      <p className={styles.message}>
        Veritas has fallen to disinformation. The Shadow Collective's network
        exploited every vulnerability — and without a population capable of
        identifying and resisting manipulated content, the city's social fabric
        has unraveled. VeraTech Solutions was only one of their shell companies.
      </p>
