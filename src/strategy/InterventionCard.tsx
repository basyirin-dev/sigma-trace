...
function tooltipText(intervention: Intervention, baseCost?: number, useCount?: number): string {
  ...
  if (effect.r0Delta < 0) lines.push(`R₀ reduction: ${effect.r0Delta} for ${effect.durationTicks}s`)
  if (effect.sigmaDelta > 0) lines.push(`Σ boost: +${effect.sigmaDelta} for ${effect.durationTicks}s`)
...
export function InterventionCard({
  ...
  const cardStyle: Record<string, React.CSSProperties> = {
    wrapper: {
      ...
      opacity: isCooldown ? 0.45 : isLocked ? 0.6 : 1,
  ...
  const statusText: string | null =
    isCooldown ? `⏳ ${cooldownRemaining}s` :
    isLocked ? 'Insufficient budget' :
    null
  ...
  const content = (
    ...
    >
      <div style={cardStyle.nameRow}>
        ...
        <span style={cardStyle.cost}>${intervention.cost}{useCount > 0 && ` (×${useCount + 1})`}</span>
      </div>
