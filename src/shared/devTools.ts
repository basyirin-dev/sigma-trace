...
export function initDevTools(): void {
  ...
  const api: DevAPI = {
    ...
    toggleDevMode() {
      ...
      console.log(`[GIHA Dev] Dev overlay: ${usePlaytestStore.getState().isDevMode ? 'ON' : 'OFF'}`)
    },
  }

  window.__GIHA_DEV__ = api
  console.log('[GIHA Dev] Dev mode active. Use window.__GIHA_DEV__ API')
  console.log('[GIHA Dev] Commands: setSigma(), setR0(), setBudget(), setTick(), unlockAll(), skipCooldowns(), state(), addIncome(), forceWin(), forceLoss(), toggleSpeed(), completeCase(), exportLog(), toggleDevMode()')
}
