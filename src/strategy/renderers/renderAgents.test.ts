...
describe('renderAgents', () => {
  it('draws pixel agents using fillRect for outline, body, highlight, and shadow', () => {
    const agents = [createAgent(10, 10, 'S'), createAgent(20, 20, 'E')];
    const { ctx, calls } = createMockCanvasContext();

    renderAgents(ctx, agents);

    const fillRectCalls = calls.filter((c) => c.method === 'fillRect');
    // Each agent: 4 fillRect calls (outline, body, highlight, shadow)
    expect(fillRectCalls.length).toBe(8);
  });

  it('sorts agents by Y coordinate before drawing', () => {
    const agents = [createAgent(10, 30, 'S'), createAgent(10, 10, 'I'), createAgent(10, 20, 'E')];
  ...
  it('draws agents with proper state colors', () => {
    const agents = [createAgent(10, 10, 'I')];
    const { ctx, calls } = createMockCanvasContext();

    renderAgents(ctx, agents);

    const fillStyleCalls = calls.filter((c) => c.method === 'fillStyle');
    // Should have set fill style for outline (#1a1a2e), state color (#E74C3C), highlight, shadow
    const outlineFill = fillStyleCalls.find((c) => c.args[0] === '#1a1a2e');
    const agentFill = fillStyleCalls.find((c) => c.args[0] === '#E74C3C');
    expect(outlineFill).toBeDefined();
