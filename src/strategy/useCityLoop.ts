...
import { createAgent, updateAgents, recolorByPopulation, createParticles, updateParticles } from './renderers'
...
export function buildDistrictState(sigma: number, r0: number, config?: SimulationConfig): Record<string, DistrictState> {
  const state: Record<string, DistrictState> = {}
  for (const district of DISTRICTS) {
    const districtR0 = config
      ? computeDistrictEffectiveR0(district, r0, config)
      : r0
