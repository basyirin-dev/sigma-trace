...
export function MetadataInspector({ metadata }: MetadataInspectorProps) {
  ...
  return (
    <div data-testid="tool-metadata-inspector" style={{ padding: '16px' }}>
      ...
      >
        ...
        {entries.map(([key, value]) => (
          <div key={key} data-testid="mi-row">
            ...
            >
              {isSuspicious(value) && <span style={{ marginRight: '6px' }}>⚠</span>}
              {value}
            </div>
          </div>
        ))}
      ...
      >
        ⚠ {entries.length} field(s) analyzed. {entries.filter(([, v]) => isSuspicious(v)).length}{' '}
        suspicious value(s) detected — highlighted in red.
      </div>
    </div>
  );
}
