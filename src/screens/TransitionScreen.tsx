...
export function TransitionScreen() {
  ...
  const direction =
    hasValidState && typeof raw.direction === 'string' ? raw.direction : 'to-strategy';
      ...
      <div className={styles.card} data-testid="transition-card">
        ...
        {(direction === 'to-strategy' || direction === 'to-gameover') && (
          <div className={styles.effects} data-testid="transition-effects">
            ...
            {sigmaDelta !== undefined && <span> Σ: +{sigmaDelta.toFixed(0)}</span>}
            {budgetBonus !== undefined && budgetBonus > 0 && <span> Budget: +{budgetBonus}</span>}
          </div>
        )}
