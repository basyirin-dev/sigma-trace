...
import { useSimulationStore, useGameStore, useWarningStore, useInterventionLogStore } from '@shared/stores'
...
export function StrategyMode() {
  ...
  const { sigma, r0, tick, isRunning, speed, simulationTick, stepTick, activeEffects } = useSimulation()
    ...
    const unsub = useGameStore.subscribe((state, prev) => {
      ...
      if (state.completedCases !== prev.completedCases) {
        ...
        if (state.completedCases === 1) {
          usePlaytestStore.getState().showMonologue('One down. Two to go. They\u2019re getting smarter.')
        }
      }
    })
    return unsub
  ...
  const handleDistrictClick = (districtId: number) => {
    ...
    const name = newSelection !== null ? districtNames[newSelection] ?? `District ${newSelection}` : 'all districts'
  ...
  const handleTick = useCallback(
    (state: CityState): CityState => {
      ...
      if (completed >= WIN_CONDITIONS.REQUIRED_CASES && sim.sigma >= WIN_CONDITIONS.SIGMA_THRESHOLD && effectiveR0 < WIN_CONDITIONS.R0_THRESHOLD) {
  ...
  useEffect(() => { handlersRef.current = { handleTogglePlay, handleStep, handleSpeedChange, handleHome } })
  ...
  return (
    ...
    >
      ...
      >
        <HUD sigma={sigma} r0={Math.max(0, r0 + useGameStore.getState().permanentR0Modifier)} budget={budget} phase={phase} lowBudget={isBudgetLow} cityPaused={cityPaused} buffs={buffs} activeEffects={activeEffects} completedCases={Object.keys(caseResults).length} r0DangerCount={r0DangerCount} r0DangerMax={30} budgetIncome={getIncome(sigma)} onRequestHome={handleHome} />
        <CoherenceGauge value={sigma} />
      </div>
      <TimeControls
        isRunning={isRunning}
        tick={tick}
      ...
      <div className={styles.mainRow}>
        <div className={styles.canvasCol}>
          <CityCanvas onDistrictClick={handleDistrictClick} onPhaseChange={setPhase} onTick={handleTick} cityPaused={cityPaused} />
        </div>
      ...
      {phaseTransition && (
        <div style={{
          ...
          background: 'rgba(0,0,0,0.55)', animation: 'fadeIn 0.3s ease-out',
        }}>
          <div style={{
            fontFamily: 'monospace', fontSize: 32, fontWeight: 'bold',
            color: phaseTransition === 'trap' ? '#E53935' : phaseTransition === 'crisis' ? '#FB8C00' : '#FFB300',
          ...
          <div style={{
            ...
            {phaseTransition === 'outbreak' ? 'Misinformation is outpacing the truth' :
             phaseTransition === 'crisis' ? 'Panic fractures neighborhoods faster than facts can unite them' :
             'Citizens have retreated into sealed echo chambers'}
          </div>
        </div>
      )}
      {showSaved && (
        <div style={{ position: 'fixed', bottom: 16, right: 16, color: 'var(--color-secondary)', fontFamily: 'monospace', fontSize: 12, background: 'rgba(0,0,0,0.6)', padding: '4px 10px', borderRadius: 4, zIndex: 9999 }}>
          Saved
        </div>
      )}
      ...
      <HintToastContainer />
      {showExitConfirm && (
        <Modal title="Leave Strategy Mode?" isOpen onClose={() => setShowExitConfirm(false)} variant="confirm" confirmLabel="Leave" cancelLabel="Stay" onConfirm={() => navigate('/', { replace: true })}>
          <p>Unsaved progress will be lost. Are you sure?</p>
        </Modal>
      )}
      {!strategyTutorialShown && (
        <StrategyTutorial onDismiss={dismissTutorial} />
      )}
    </div>
  )
}
