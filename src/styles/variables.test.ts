...
describe('variables.css design tokens', () => {
  ...
  describe('spacing scale', () => {
    ...
    it('values are in ascending order', () => {
      const values = ['space-xs', 'space-sm', 'space-md', 'space-lg', 'space-xl', 'space-2xl'].map(
        (n) => parseInt(extractVar(n) ?? '0'),
      );
  ...
  describe('z-index scale', () => {
    ...
    it('values are in ascending order', () => {
      const values = ['z-canvas', 'z-content', 'z-overlay', 'z-hud', 'z-modal', 'z-tooltip'].map(
        (n) => parseInt(extractVar(n) ?? '0'),
      );
      for (let i = 1; i < values.length; i++) {
        expect(values[i]!).toBeGreaterThan(values[i - 1]!);
  ...
  describe('line-height', () => {
    it('defines --leading-tight: 1.3', () => {
      expect(extractVar('leading-tight')).toBe('1.3');
    });

    it('defines --leading-normal: 1.55', () => {
      expect(extractVar('leading-normal')).toBe('1.55');
    });

    it('defines --leading-relaxed: 1.8', () => {
      expect(extractVar('leading-relaxed')).toBe('1.8');
    });
  });
