...
function renderToolModal(toolId: string, evidenceId: string, caseData: CaseData, caseId: string) {
...
export function DetectiveMode() {
  ...
  useEffect(() => {
    ...
    if (
      caseId === 'case-01' &&
      toolModal.toolId === 'metadata-inspector' &&
      toolModal.evidenceId === 'evidence-01'
    ) {
    ...
    if (caseId === 'case-03' && toolModal.evidenceId === 'evidence-05') {
      usePlaytestStore
        .getState()
        .showMonologue('Oh no. She\u2019s not a villain. She\u2019s a victim.');
    }
  }, [toolModal, caseId]);
  ...
  const handleReturnToCity = useCallback(() => {
    ...
    navigate('/transition', {
      state: {
        ...
        verdict:
          score.grade === 'F' ? (playerVerdict ?? 'uncertain') : caseData.meta.correctVerdict,
  ...
  const handleToolApply = useCallback(
    (evidenceId: string, toolId: string) => {
      ...
      if (allowedTypes && !allowedTypes.includes(evidence.type)) {
        useWarningStore
          .getState()
          .addWarning(`${toolId} doesn't work on ${evidence.type} evidence. Try a different tool.`);
        return;
      }
  ...
  return (
    ...
    >
      <CutscenePlayer frames={caseData.script.introCutscene} caseTitle={caseData.meta.title} />
        ...
        (() => {
          ...
          return (
            <ToolTutorialOverlay
              ...
              isOpen
              onDismiss={() => {
                dismissTutorial(tutorialToolId);
                setTutorialToolId(null);
              }}
            />
          );
        })()}
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div>
                      <div
                        style={{ fontFamily: 'var(--font-pixel)', fontSize: 17, color: '#e0e0e0' }}
                      >
                        {tool.label}
                      </div>
